...
export async function CaseStudyView({
  ...
  // Named cover morph only on full-page navigations. Intercepted modals keep
  // the portfolio mounted underneath, so a shared name would duplicate.
  const cover = (
  ...
  return (
    <ViewTransition enter="slide-up" default="none">
      <article className={mode === "modal" ? "max-h-[90vh] overflow-y-auto" : ""}>
        {mode === "page" ? (
          <ProjectCoverTransition slug={project.slug}>{cover}</ProjectCoverTransition>
        ) : (
          cover
        )}
